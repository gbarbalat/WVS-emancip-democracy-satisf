...
rfx <- "(1+Y020|COUNTRY_ALPHA)" # "(1|COUNTRY_ALPHA)" "(1+Y020|COUNTRY_ALPHA)"
survey_anal <- 7
...
#scale numeric columns (important for gdp or ldi)
# ultimate_db_filtered <- ultimate_db_filtered %>%
#   mutate(across(where(is.numeric), scale))
#   #mutate(across(matches("ldi|gdp"), scale))


#make A170 (life satisfaction on the WVS) numeric
